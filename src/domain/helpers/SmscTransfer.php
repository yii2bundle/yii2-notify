...
class SmscTransfer
{
    ...
    public $POST = 0;                    // использовать метод POST
    public $HTTPS = 0;                // использовать HTTPS протокол
    public $CHARSET = "utf-8";    // кодировка сообщения: utf-8, koi8-r или windows-1251 (по умолчанию)
    public $DEBUG = 0;                // флаг отладки
    public $SMTP_FROM = "api@smsc.ru";     // e-mail адрес отправителя
     ...
     * @param int $format формат сообщения (0 - обычное sms, 1 - flash-sms, 2 - wap-push, 3 - hlr, 4 - bin, 5 - bin-hex, 6 - ping-sms, 7 - mms, 8 - mail, 9 - call, 10 - viber)
     ...
     * @return bool
     */
    function sendMail($phones, $message, $translit = 0, $time = 0, $id = 0, $format = 0, $sender = "")
    {
        return mail("send@send.smsc.ru", "", $this->LOGIN . ":" . $this->PASSWORD . ":$id:$time:$translit,$format,$sender:$phones:$message", "From: " . SMTP_FROM . "\nContent-Type: text/plain; charset=" . CHARSET . "\n");
    }
     ...
     * @return array
     */
    private function sendCmd($cmd, $arg = "", $files = array())
    {
     ...
     */
    private function readUrl($url, $files, $tm = 5)
    {
        $ret = "";
...
// Examples:
// list($sms_id, $sms_cnt, $cost, $balance) = send("79999999999", "Ваш пароль: 123", 1);
// list($sms_id, $sms_cnt, $cost, $balance) = send("79999999999", "http://smsc.ru\nSMSC.RU", 0, 0, 0, 0, false, "maxsms=3");
// list($sms_id, $sms_cnt, $cost, $balance) = send("79999999999", "0605040B8423F0DC0601AE02056A0045C60C036D79736974652E72750001036D7973697465000101", 0, 0, 0, 5, false);
// list($sms_id, $sms_cnt, $cost, $balance) = send("79999999999", "", 0, 0, 0, 3, false);
// list($sms_id, $sms_cnt, $cost, $balance) = send("dest@mysite.com", "Ваш пароль: 123", 0, 0, 0, 8, "source@mysite.com", "subj=Confirmation");
// list($cost, $sms_cnt) = getSmsCost("79999999999", "Вы успешно зарегистрированы!");
// sendMail("79999999999", "Ваш пароль: 123", 0, "0101121000");
